...
@app.get("/")
def index():
    """Serve the triage console UI (a demo client for /sort-ticket)."""
    try:
        with open(_INDEX_PATH, encoding="utf-8") as fh:
            return Response(fh.read(), mimetype="text/html")
    except OSError:
        # UI is optional; the API still works without it.
        return jsonify({"service": "queuestorm", "status": "ok",
                        "endpoints": ["GET /health", "POST /sort-ticket"]}), 200
